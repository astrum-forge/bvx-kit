...
export class VoxelChunk16 extends VoxelChunk {
    ...
    /**
     * Number of bytes the meta-data storage occupies (16 bits per voxel).
     */
    public static readonly META_BYTE_LENGTH: number = (VoxelChunk.SIZE * 16) / 8;
    ...
    constructor(key: MortonKey, storage: ChunkStorage | null = null) {
        ...
        // Allocate or view enough space for 16 bits of meta-data per voxel in the chunk (64 voxels).
        if (storage !== null) {
            this._metaDataBuffer = storage.buffer;
            this._metaData = new Uint16Array(storage.buffer, storage.metaByteOffset, VoxelChunk.SIZE);
        }
        else {
            this._metaDataBuffer = new ArrayBuffer(VoxelChunk16.META_BYTE_LENGTH);
